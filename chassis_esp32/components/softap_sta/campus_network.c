...
static EventGroupHandle_t can_ev = NULL;
#define AUTH_SUCCESSFULLY   (BIT0)
#define AUTH_FAILED         (BIT1)
...
static void can_http_result(const char* cjson_str)
{
    /* 解析根对象 */
    cJSON* js_root = cJSON_Parse(cjson_str);

    /* 获取result */
    cJSON* result_js = cJSON_GetObjectItem( js_root, "result");
    /* 获取对应值 */
    const char* result_str = cJSON_GetStringValue( result_js);
    ESP_LOGI( TAG, "id: %s", result_str);
    if( 0 == strcmp( result_str, "success") ) {
        ESP_LOGI(TAG, "校园网认证成功, 结果: %s", result_str);
        xEventGroupSetBits( can_ev, AUTH_SUCCESSFULLY);
    } else {
        /* 获取message */
        cJSON* message_js = cJSON_GetObjectItem( js_root, "message");
        ESP_LOGI(TAG, "校园网认证失败, 结果: %s, 失败原因: %s", result_str, cJSON_GetStringValue( message_js));
        xEventGroupSetBits( can_ev, AUTH_FAILED);
    }
...
uint8_t campus_network_http_post( login_info_t* user_info)
{
    ...
    uint8_t ret = 0;    //  默认返回 0
    if( err == ESP_OK ){
        int status_code = esp_http_client_get_status_code(http_client);
        if (status_code >= 200 && status_code < 300) {  //  状态码正常时
            ret = 1;    
        } 
    } 
    ...
    EventBits_t ev = xEventGroupWaitBits( can_ev, AUTH_SUCCESSFULLY | AUTH_FAILED, pdFALSE, pdFALSE, portMAX_DELAY);

    vEventGroupDelete( can_ev);
    if( ev & AUTH_SUCCESSFULLY )
        return 1;
    else 
        return 0;
}
